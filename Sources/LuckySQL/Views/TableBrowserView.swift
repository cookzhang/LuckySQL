        ...
        if let table = model.selectedTable {
            VStack(spacing: 0) {
                HStack(spacing: 10) {
                    ...
                    TextField("Filter value", text: $model.browseOptions.filterValue).textFieldStyle(.roundedBorder)
                ...
                HStack(spacing: 12) {
                    ...
                    Text("Page \(model.browseOptions.page + 1)").font(.caption).monospacedDigit()
...
struct QueryHistoryView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            TextField("Search SQL, database or connection", text: $search).textFieldStyle(.roundedBorder)
