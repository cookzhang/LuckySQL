...
final class MySQLConnectionHandler: ChannelDuplexHandler {
    ...
    struct HandshakeState {
        ...
        let additionalCertificateVerification: (@Sendable (NIOSSLCertificate, any Channel) -> EventLoopFuture<Void>)?
    ...
    func handleHandshake(context: ChannelHandlerContext, packet: inout MySQLPacket, state: HandshakeState) throws {
        ...
        if state.tlsConfiguration != nil && !handshakeRequest.capabilities.contains(.CLIENT_SSL) {
            throw MySQLError.unsupportedServer(message: "TLS is required but the server does not support it; authentication was not sent")
        }
        ...
        if let tlsConfiguration = state.tlsConfiguration, handshakeRequest.capabilities.contains(.CLIENT_SSL) {
            ...
            promise.futureResult.assumeIsolated().flatMapThrowing {
                ...
                let handler: NIOSSLClientHandler
                if let verify = state.additionalCertificateVerification {
                    handler = try NIOSSLClientHandler._makeSSLClientHandler(context: sslContext, serverHostname: state.serverHostname, additionalPeerCertificateVerificationCallback: verify)
                } else {
                    handler = try NIOSSLClientHandler(context: sslContext, serverHostname: state.serverHostname)
                }
    ...
    func writeHandshakeResponse(
        ...
        if let version = SemanticVersion(string: versionString) {
            if !handshakeRequest.serverVersion.contains("MariaDB") {
                ...
                case (5, 6...), (6..., _): // LuckySQL validates 5.6 and newer with explicit legacy collation
                    break
                default:
                    self.logger.error("Unsupported MySQL version: \(handshakeRequest.serverVersion)")
                    self.logger.info("LuckySQL validates MySQL 5.6 or higher")
                }
            }
        } else {
    ...
    func channelInactive(context: ChannelHandlerContext) {
        switch state {
        case .handshake(let state): state.done.fail(MySQLError.closed)
        case .authenticating(let state): state.done.fail(MySQLError.closed)
        case .commandPhase: break
        }
